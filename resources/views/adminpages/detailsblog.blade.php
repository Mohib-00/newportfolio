...
<div id="loader" style="display: none">
   ...
   $(document).on('click', '.delblog', function() {
       ...
       $('#loader').show();
       $.ajax({
           url: "{{ route('detailblogssssss.store') }}",
           type: "POST",
           data: formData,
           contentType: false,
           processData: false,
   ...
   $(document).on('click', '.edit-blog-btn', function () {
       ...
       $('#loader').show();
       $.ajax({
           url: "{{ route('detailblogssssss.show', '') }}/" + blogId, 
           type: "GET",  
           success: function (response) {
   ...
   $('#blogeditform').on('submit', function (e) {
       ...
       $.ajax({
           url: "{{ route('detailblogssssss.update', '') }}/" + blogId,  
           type: "POST",  
           data: formData,
           contentType: false, 
           processData: false, 
           success: function (response) {
               ...
               if (response.success) {
                   Swal.fire({
                       ...
                       blog.find('td:nth-child(5)').text(response.blog.slug);
                   });
               } else {
                   Swal.fire({
                       icon: 'error',
